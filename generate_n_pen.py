...
def create_matlab(p_length, p_mass, t_init, tv_init, time_length):
	...
	with open("n_pen.m", 'w') as f:
		...
		for i in range(n_len):
			...
			if i == len(t_init)-1:
				...
				init_string += str(tv_init[i])+"; "
		f.write("t_interval = 0:0.025:"+str(time_length)+";\n")
		f.write(init_string+"\n")
		...
		f.write("v = VideoWriter(\'n"+str(n_len)+"_pen.avi\');\n")
		f.write("open(v);\n")
		...
		f.write("\tframe = getframe;\n")
		f.write("\twriteVideo(v, frame);\n")
		f.write("end\n")
...
if __name__ == '__main__':
	l_in = [1, 1, 1, 1, 1, 1];
	m_in = [10, 10, 10, 10, 5, 9];
	t_in = ["pi/2", "pi/2", "pi/2", "pi/4", "pi/3", "pi/2"];
	tv_in = [0, 0, 0, 0, 0, 0];
	time = 10;

	create_matlab(l_in, m_in, t_in, tv_in, time)
